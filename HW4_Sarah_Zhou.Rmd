...
My shiny app allows the user to choose an explanatory categorical variable from a drop-down menu and a responding variable (fill) from another drop-down menu. Using ggplot, the result will be a histogram with the explanatory variable graphed on the x-axis, the responding variable as the fill, and the count graphed on the y-axis. 

```{r}
shinyUI(fluidPage(
titlePanel("Categorical Analysis on Duke Basketball Data"),
sidebarLayout(
    sidebarPanel(

      selectInput("expvar", 
            "Explanatory Variable (Categorical):",
            choices = c("result", "season", "result","location", "month")),
...
output$categoricalPlot <- renderPlot({
  ...
  basketball <- basketball %>%
    filter(!is.na(input$expvar)) %>%
    filter(!is.na(input$color))
  
    ggplot(data = basketball, aes_string(x = input$expvar, fill = input$color)) +
      geom_bar() +
      ggtitle(paste0(input$expvar," vs. ",input$color))
...
#   
#     ggplot(data = basketball, aes_string(x = input$numvar1, y = input$numvar2, fill = input$color2)) +
#       geom_point() +
#       ggtitle(paste0(input$numvar1," vs. ",input$numvar2))
# 
# 
#   })
